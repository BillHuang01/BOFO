...
for (l in 1:30){
  ...
  print("Surrogate model on log MSE")
  logmse <- log(mse)
  
  print("Fit Surrogate Model...")
  invisible(capture.output(model <- mlegp(D[-rm.idx,], logmse[-rm.idx])))
  
  print("Find Next Sample by EI...")
  mse.min <- min(mse)
  logmse.min <- min(logmse)
  EI <- function(x){
    x <- exp(x) / (1 + exp(x)) # undo logit transform
    logmse.est <- predict(model, x, se.fit = T)
    mu <- logmse.est$fit
    sigma <- logmse.est$se.fit
    if (sigma < 1e-9) return (0)
    val <- pnorm((logmse.min-mu)/sigma)*mse.min - 
      exp(mu+sigma^2/2)*pnorm((logmse.min-mu-sigma^2)/sigma)
...
unlink("logstandard_new_design.txt")
